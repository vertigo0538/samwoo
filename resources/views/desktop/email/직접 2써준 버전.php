...
$(function(){
	...
	function addField() {
		...
		clone = '<div class="select_box cf">'+
				...
				'					<p>특장차</p>'+
				'				</label>'+
				'				<input id="select1" value="special" type="radio" name="main2">'+
				'			</li>'+
				'			<li>'+
				'				<label for="select2">'+
				'					<img src="{{URL::asset('images/quota/round.png')}}" alt="">'+
				'					<p>상용트럭</p>'+
				'				</label>'+
				'				<input id="select2" value="common" type="radio" name="main2">'+
				'			</li>'+
				'			<li>'+
				'				<label for="select3">'+
				'					<img src="{{URL::asset('images/quota/round.png')}}" alt="">'+
				'					<p>건자재</p>'+
				'				</label>'+
				'				<input id="select3" value="construction" type="radio" name="main2">'+
				'			</li>'+
				'		</ul>'+
				'	</div>'+
...
// 1뎁스결정하기
	$('.select_wrap').delegate('input[name=main],input[name=main2]','change',function(){
		var _this = $(this),
		     val = _this.val(),
		     div = _this.closest('.select_box'),
		     sel = div.find('select[name=onedepth],select[name=onedepth2]');
...
// 2차뎁스결정하기
	$('.select_wrap').delegate('select[name=onedepth],select[name=onedepth2]','change',function(){
		var _this = $(this),
		     val = _this.val(),
		     div = _this.closest('.select_box'),
		     sel = div.find('select[name=twodepth],select[name=twodepth2]'),
		     sel3 = div.find('select[name=threedepth],select[name=threedepth2]');
...
// 3차뎁스 결정하기
	$('.select_wrap').delegate('select[name=twodepth],select[name=twodepth2]','change',function(){
		var _this = $(this),
		    val = _this.val(),
		    div = _this.closest('.select_box'),
		    sel = div.find('select[name=threedepth],select[name=threedepth2]');
		    sel1 = div.find('select[name=onedepth],select[name=onedepth2]');
...
//4차뎁스 결정하기
	...
	$('.select_wrap').delegate('select[name=threedepth],select[name=threedepth2]','change',function(){
		var _this = $(this),
		    val = _this.val(),
		    div = _this.closest('.select_box'),
		    sel = div.find('select[name=fourdepth],select[name=fourdepth2]');
		    sel1 = div.find('select[name=onedepth],select[name=onedepth2]');
		    sel2 = div.find('select[name=twodepth],select[name=twodepth2]');
...
// textarea 노출
$('.select_wrap').delegate('select[name=fourdepth],select[name=fourdepth2]','change',function(){
	var _this = $(this),
			val = _this.val();
	switch(val){
		case "기타(별도기재부탁드립니다)" :
